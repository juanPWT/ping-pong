...
pub const Ai = struct {
    ...
    pub fn init(x: f32, y: f32, sizeX: f32, sizeY: f32, speed: f32, color: rl.Color, score: u8) Ai {
        return Ai{
            .position = rl.Vector2.init(x, y),
            .size = rl.Vector2.init(sizeX, sizeY),
            .speed = rl.Vector2.init(speed, speed),
    ...
    pub fn update(self: *Ai, ball: *Ball) void {
        // move to ball position
        if (ball.position.y <= self.position.y) {
            // move up
            if (self.position.y >= 0) self.position.y -= self.speed.y;
        } else if (ball.position.y >= self.position.y + self.size.y) {
            // move down
            if (self.position.y + self.size.y <= SCREEN_HEIGHT) self.position.y += self.speed.y;
        }
    }

    pub fn draw(self: *Ai) void {
        rl.drawRectangleV(self.position, self.size, self.color);
    }
};
